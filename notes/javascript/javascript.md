...
Garbage collection = automatically removing unused objects from memory.
Algorithm used for garbage-collection: Mark-and-sweep algorithm
  - Mark: The object which are reachable from the root are marked as 'traceable'
  - Sweep: Delete un-marked references

Memory-Leak: When there are objects in the memory which are no longer required and they are incorrectly still reachable, then it is not garbage collected causing memory leaks.
...
const setArr = Array.from(set); or [...set] //convert set to array

//new methods
let arr1 = ['ram','shyam', 'riya'];
let arr2 = ['rohan','mohan','riya'];

let set1 = new Set(arr1);
let set2 = new Set(arr2);
console.log(set1.intersection(set2)); // unique common across 2 sets
console.log(set1.union(set2)); // combining all unique elements from both sets.
console.log(set1.difference(set2)); // Elements that are in Set 1 but NOT in Set 2
console.log(set1.symmetricDifference(set2)); // elements that are in either set, but not in both
...
Polyfills - It's a piece of code that is used to support mordern JS features in older browsers that donot support those modern features natively.
