...
// toSpliced() -> doesnont mutate the original array
...
// GROUPBY - Groups elements based on the key returned from callback

const arr = [1, 2, -3, 4, -5, 6];

const resultObj = Object.groupBy(arr,(element, index) => {
  return element < 0 ? 'deposits' : 'withdrawals';
});
...
// toReversed() -> doesnont mutate the original array
...
console.log(arr.flatMap()); // use flat(1) on an array after using map()
...
console.log(Array(7).fill('a', 2, 5)); // [empty × 2, 'a', 'a', 'a', empty × 2] // .fill (.fill(value, start(inclusive), end(exclusive)))
console.log(Array(5, 4, 5, 6, 7).fill('a', 1, 4)); // [5, 'a', 'a', 'a', 7]

//FROM

const x = Array.from({ length: 10 }, () => Math.floor(Math.random() * 7));
console.log(x); // [2, 2, 0, 6, 5, 5, 6, 6, 4, 3]
...
// parseInt() reads until it hits a non-numeric character.
...
//Number.isNaN() only returns true if the value is actually the special JavaScript value NaN and its type is number.
...
// isNaN() coerces values to numbers first

console.log(isNaN(23)); // false
console.log(isNaN('23')); // false
console.log(isNaN('23px')); // true
console.log(isNaN('px23')); // true
console.log(isNaN(+'23px')); // true
console.log(isNaN(+'px23')); // true
